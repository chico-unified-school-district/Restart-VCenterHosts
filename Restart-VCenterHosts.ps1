...
foreach ($server in $VIServers) {
 ...
 foreach ($cluster in $allTargetClusters) {
  if ($SkipClusters -contains $cluster.name) {
   Add-Log skipcluster ('{0}' -f $cluster.name)
   continue
  }
  else {
   ...
   try {
    $cluster |
    Set-Cluster -HAEnabled:$false -Confirm:$false -ErrorVariable clusterHAError -WhatIf:$WhatIf |
    Out-Null
   }
   catch {
   ...
   Add-Log cluster ('{0},Disable HA successfull' -f $targetName) -WhatIf:$WhatIf
   ...
   if ($DrsAutomationLevel -ne 'FullyAutomated') {
    try {
     $cluster |
     Set-Cluster -DrsAutomationLevel FullyAutomated -Confirm:$false -ErrorVariable clusterDRSError -WhatIf:$WhatIf |
     Out-Null
    }
    catch {
   ...
   foreach ($esxiHost in $allClusterHosts) {
    if ($SkipHosts -contains $esxiHost.name) {
     ...
     if ( (Get-VMHost $vmHostName).ConnectionState -eq 'Maintenance') {
      ...
      Add-Log wait ('{0},30 minutes max wait for host evacuation' -f $vmHostName) -WhatIf:$WhatIf
      do {
       Start-Sleep 1
     ...
     Add-Log hostwait ('{0}, rebooting' -f $vmHostName) -WhatIf:$WhatIf
     do {
   ...
   Add-Log storage ('{0} - {1},Waiting for storage' -f $server, $cluster.name) -WhatIf:$WhatIf
   if (!$WhatIf) {
    for ($i = 180; $i -ge 0; $i--) {
